...
function setupChargeTimeSlider() {
  ...
  function selectIndex(index, updateHash) {
    ...
    updateRoadCoverage(layerKeyForIndex(index));
...
function updateRoadCoverage(key) {
  const value = document.getElementById("road-pct");

  if (!value) return;
  const road = manifest.road_coverage;
  const layer = manifest.layers[key];
  if (!road || !layer || layer.road_pct == null) return;

  value.textContent = layer.road_pct.toFixed(1) + "%";
  document.getElementById("road-hint").textContent =
    layer.road_covered_km.toLocaleString() + " km of " +
    road.total_km.toLocaleString() + " km of Australia's public roads within reach.";
}
